...
class GitManager:
    ...
    def is_repo_exists(self) -> bool:
        """
        仅判断目录存在不够：Windows 上曾出现 cache 目录残留一个“空 .git 目录”，
        git 会忽略它并向上级目录查找 .git，导致把父仓库当成当前仓库来操作，
        最终出现“未检测到 pom.xml / 扫描结果全为 0”。
        """
        if not os.path.isdir(self.git_dir):
            return False
        head = os.path.join(self.git_dir, "HEAD")
        try:
            return os.path.isfile(head) and os.path.getsize(head) > 0
        except Exception:
            return False
    ...
    def clone(self, repo_url: str, branch: str = "master", shallow: bool = True, skip_fsck: bool = True, timeout: int = 600, git_config: Dict[str, str] = None) -> Tuple[bool, str]:
        ...
        try:
            ...
            # 若目标目录已存在但不是有效 git 仓库（例如残留空 .git），先清理，否则 git clone 会失败或走错仓库
            if os.path.isdir(self.repo_path) and (not self.is_repo_exists()):
                try:
