...
class MethodAnalyzer(BaseAnalyzer):
    ...
    def _extract_parameters(self, node: ExtractedNode, method: MethodInfo, context: AnalyzerContext) -> List[ParameterInfo]:
        ...
        param_nodes = AstTool.iter_children(params_node)
    ...
    def _extract_exceptions(self, node: ExtractedNode) -> List[str]:
        ...
        if throws_node:
            # find_child_by_type 返回单个节点，不能直接迭代；这里改为“找所有类型节点”
            for tn in AstTool.find_child_by_types(
                throws_node,
                [JavaAstNodeType.TYPE_IDENTIFIER.value, JavaAstNodeType.GENERIC_TYPE.value],
                deep=True,
            ):
