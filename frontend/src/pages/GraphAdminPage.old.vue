...
const publishRecordLoadingProject = ref<string | null>(null)
...
async function openPublishRecord(record: any) {
  const projectName = String(record?.project_name || '')
  publishRecordLoadingProject.value = projectName
  try {
    // 如果还没有加载过任务列表，先加载一次
    if (!importTasks.value || importTasks.value.length === 0) {
    ...
    publishRecordLoadingProject.value = null
  }
}
...
onMounted(() => {
  void fetchProjects()
  // 不再在 onMounted 时加载诊断数据，改为切换到诊断 tab 时才加载
  // void fetchDiagnostics()
  // 不再在概览页面自动加载导入任务，改为点击【查看】时才加载
  // void fetchImportTasks()
  void fetchRepos()
  importPollTimer = window.setInterval(async () => {
    // 只在导入/重建 tab 激活时才轮询
    if (active.value === 'import') {
      await fetchImportTasks()
...
// 监听 tab 切换，只在切换到诊断 tab 时才加载诊断数据
watch(
  () => active.value,
  (newActive) => {
  ...
  { immediate: true }, // immediate: true 保证首次加载时也会检查（但此时 active='overview'，不会触发）
)
                ...
                <template #bodyCell="{ column, record }">
                  ...
                  <template v-else-if="column.key === 'release_record'">
                    <a-button 
                      type="link" 
                      size="small" 
                      :loading="publishRecordLoadingProject === record.project_name"
                      @click.stop="openPublishRecord(record)"
                    >
