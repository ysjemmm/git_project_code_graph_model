...
logger = get_logger("java_adapter")
    ...
    def parse_files(
        ...
        for file_path in source_files:
            try:
                ...
                if ast_data is None:
                    raise ValueError("analyze_file returned None")
                ...
                if len(fail_samples) < 8:
                    ...
                    try:
                        ...
                        rel = file_path
                    tb = traceback.format_exc(limit=30).strip().replace("\n", " | ")
                    fail_samples.append((rel, f"{type(e).__name__}: {e} | {tb}"))

        if parsed_fail:
            logger.warning(
                f"[PARSE] Java 解析完成：成功 {parsed_ok}，失败 {parsed_fail}（仅展示前 {len(fail_samples)} 条失败样本）"
            )
            for rel, msg in fail_samples:
                logger.warning(f"[PARSE] 失败样本: {rel} -> {msg}")
        else:
            logger.info(f"[PARSE] Java 解析完成：成功 {parsed_ok}，失败 0")
