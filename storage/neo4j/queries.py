...
class Neo4jQueries:
    ...
    @staticmethod
    def merge_same_artifact_links() -> str:
        """
        ...
        """
        return """
        MATCH (lib:Project {project_type: 'Lib'}), (app:Project {project_type: 'Application'})
        WHERE lib.name = app.name
        MERGE (lib)-[r:SAME_ARTIFACT]->(app)
        ON CREATE SET r.__tmp_created = 1
        WITH r, coalesce(r.__tmp_created, 0) AS is_new
        SET r.__tmp_created = null
        WITH sum(is_new) AS created_count
