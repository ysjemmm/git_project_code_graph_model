...
public class BizDemandDetailVO extends ToString {
    ...
    @ApiModelProperty("关联线上bug列表")
    private List<BugOnlineLinkVO> bugOnlineList;

    @ApiModelProperty("线下bug id")
    @JsonSerialize(using = ToStringSerializer.class)
    private Long bugOfflineId;

    @ApiModelProperty("线下bug 名称")
    ...
    @ApiModelProperty("是否客开需求")
    private String customerDevDemandText ;

    @ApiModelProperty("客开类型：0基于销售合同；1基于项目")
    private Integer customerDevType;

    @ApiModelProperty("客开类型")
    private String customerDevTypeText ;
