...
- **反编译兜底**：当遇到三方包（如 Spring、MyBatis、Guava）或图谱中不存在的二方包，且需要了解其具体实现或方法签名时，应调用 decompile_class 工具对目标类进行反编译。反编译结果可能是完整源码（CFR）或方法签名（javap），均可用于分析调用行为、参数类型、返回值等。不要因为"没有源码"就放弃深入排查。
